...
import { lessonGroups } from './levels.js?v=1.0.9';
...
const ICONS = {
    ...
    jumpForward: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 14L12 4M12 4L6 10M12 4L18 10" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 20C4 20 6 18 8 18C10 18 11 20 13 20C15 20 16 18 18 18C20 18 21 20 21 20" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
...
async function runCode() {
    ...
    characterState = { ...level.start, rotation: 0, failed: false };
    ...
    for (const command of commandQueue) {
        if (characterState.failed) break;

        let nextRow = characterState.row, nextCol = characterState.col;
        const { dir } = characterState;

        if (command === 'moveForward' || command === 'jumpForward') {
            if (dir === 0) nextRow++;
        ...
        const isOutOfBounds = nextRow < 0 || nextRow >= level.gridSize.rows || nextCol < 0 || nextCol >= level.gridSize.cols;
        const nextTileType = isOutOfBounds ? ' ' : level.layout[nextRow][nextCol];
        
        if (command === 'moveForward') {
            if (isOutOfBounds || nextTileType !== 'p') {
        ...
        } else if (command === 'jumpForward') {
            const currentY = character.position.y;
            const targetPos = gridToWorld(nextRow, nextCol, level.gridSize);
            await animateTo(character.position, { y: currentY + JUMP_HEIGHT }, 250, TWEEN.Easing.Quadratic.Out);
            await animateTo(character.position, { x: targetPos.x, z: targetPos.z }, 250);
            await animateTo(character.position, { y: currentY }, 250, TWEEN.Easing.Quadratic.In);
            
            if (isOutOfBounds || nextTileType !== 'p') {
                characterState.failed = true;
            } else {
                characterState.row = nextRow;
                characterState.col = nextCol;
            }
    ...
    if (characterState.failed) {
        await animateTo(character.position, { y: -10 }, 500); // Animación de caída
    }
    
    const gemsCollected = gems.filter(g => g.userData.collected).length;
    const isCorrect = !characterState.failed && (JSON.stringify(commandQueue) === JSON.stringify(level.solution)) && (gemsCollected === level.gems.length);
